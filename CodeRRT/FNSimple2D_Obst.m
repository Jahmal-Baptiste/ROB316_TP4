    ...
    methods
        ...
        function position = sample(this)
            ...
            if rand() < 0.05
                ...
                obstacle_index = randi(this.obstacle.num);
                
                target_point   = this.obstacle.output{obstacle_index}(randi(size(this.obstacle.output{obstacle_index}, 1)), :);
                
                while ((target_point(1) == this.XY_BOUNDARY(1)) || (target_point(1) == this.XY_BOUNDARY(2)) || ...
                        (target_point(2) == this.XY_BOUNDARY(3)) ||(target_point(2) == this.XY_BOUNDARY(4)))
                    target_point = this.obstacle.output{obstacle_index}(randi(size(this.obstacle.output{obstacle_index}, 1)), :);
                end
                
                position       = [target_point(1) + (2*rand()-1)*region_size; target_point(2) + (2*rand()-1)*region_size];
                
                if position(1) < this.XY_BOUNDARY(1)
                    position(1) = this.XY_BOUNDARY(1);
                elseif position(1) > this.XY_BOUNDARY(2)
                    position(1) = this.XY_BOUNDARY(2);
                end
                if position(2) < this.XY_BOUNDARY(3)
                    position(2) = this.XY_BOUNDARY(3);
                elseif position(2) > this.XY_BOUNDARY(4)
                    position(2) = this.XY_BOUNDARY(4);
                end
            else    
